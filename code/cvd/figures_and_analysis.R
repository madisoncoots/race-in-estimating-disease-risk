...
# ======================
# Code for all cardiovascular figures, tables, and statistics.
# Coots et al. (2024)
...
data_object_read_path <- directory_path %>% str_remove("code/cvd") %>% str_c("data/cvd/processed/cvd_data_with_risk.rds")
...
cvd_race_blind_calib <- data %>%
  ...
  mutate(disease = "Cardiovascular disease") %>% # For eventual faceting
...
utility_gains_by_group <- 
  data %>% 
  mutate(p_disease = race_aware_ascvd_risk,
         race_aware_risk_score = race_aware_ascvd_risk,
         race_blind_risk_score = race_blind_ascvd_risk,
...
cvd_utility_dot_plot <- utility_gains_by_group %>%
  mutate(disease = "Cardiovascular disease") %>% # For easier faceting
...
cvd_dot_plot <- pct_changed_by_group %>%
  mutate(disease = "Cardiovascular disease") %>% # For easier faceting
...
compute_blind_scarcity_thresh_using_pct <- function(data, pct_ppl_we_can_screen) {
  data %>%
    mutate(risk_score = race_blind_ascvd_risk) %>%
    drop_na(risk_score) %>%
...
compute_aware_scarcity_thresh_using_pct <- function(data, pct_ppl_we_can_screen) {
  data %>%
    mutate(risk_score = race_aware_ascvd_risk) %>%
    drop_na(risk_score) %>%
    arrange(desc(risk_score)) %>%
    mutate(cumulative_sum = cumsum(wtmec8yr),
           cumulative_pct = cumulative_sum / sum(wtmec8yr) * 100) %>%
...
cvd_scarcity_utility_plot <- scarcity_utility_plot_data %>%
  filter(race_blind_scarcity_thresh > screening_thresh) %>%
  mutate(disease = "Cardiovascular disease") %>%
...
for (c in capacities) {
  ...
  race_aware_scarcity_thresh <- compute_aware_scarcity_thresh_using_pct(data, c)
  pct_changed_by_group <- data %>%
    drop_na(race_blind_ascvd_risk, race_aware_ascvd_risk) %>%
    mutate(race_blind_screening_decision = race_blind_ascvd_risk > max(screening_thresh, race_blind_scarcity_thresh),
           race_aware_screening_decision = race_aware_ascvd_risk > max(screening_thresh, race_aware_scarcity_thresh),
           changed_decision = (race_blind_screening_decision != race_aware_screening_decision)) %>%
    group_by(race) %>%
    summarize(pct_changed_decision = sum(changed_decision * wtmec8yr) / sum(wtmec8yr),
              n = sum(wtmec8yr)) %>%
...
cvd_scarcity_pct_changed_plot <- scarcity_pct_changed_plot_data %>%
  filter(race_blind_scarcity_thresh > screening_thresh) %>%
  mutate(disease = "Cardiovascular disease") %>%
...
cvd_baseline_utility_plot <- baseline_utility_by_group %>%
  mutate(disease = "Cardiovascular disease") %>% # For easier faceting
...
subgroup_aware_data <- data %>%
  mutate(model = "Cardiovascular disease\n\nRace-aware") %>%
  mutate(decision = race_aware_ascvd_risk > screening_thresh,
         detection = decision * race_aware_ascvd_risk) %>% # aware is "true" risk
  filter(detection > screening_thresh)

subgroup_blind_data <- data %>%
  mutate(model = "Cardiovascular disease\n\nRace-unaware") %>%
  mutate(decision = race_blind_ascvd_risk > screening_thresh,
         detection = decision * race_aware_ascvd_risk) %>% # aware is "true" risk
  filter(detection > screening_thresh)
...
subgroup_means <- subgroup_data %>%
  group_by(model, race) %>%
  summarize(mean_age = sum(ridageyr * wtmec8yr) / sum(wtmec8yr))

cvd_subgroup_histogram <- 
  subgroup_data %>%
  mutate(race = factor(race, levels = c("Asian", "Black", "Hispanic", "White"))) %>%
  mutate(disease = "Cardiovascular disease") %>%
  ggplot(aes(x = ridageyr, y = ..density.., color = race, weight=wtmec8yr)) +
...
for (k in benefit_range) {
  result <- 
    ...
    mutate(new_threshold = optimal_thresh / k,
           aware_screened = race_aware_ascvd_risk > new_threshold,
           detection = aware_screened * race_aware_ascvd_risk) %>% 
    group_by(race) %>%
    summarize(num_aware_screened = sum(aware_screened * wtmec8yr), 
              ...
              frac_aware_detections = num_aware_detections / sum(wtmec8yr * race_aware_ascvd_risk),
...
status_quo_blind <- data %>%
  mutate(disease = "Cardiovascular disease",
         blind_screened = race_blind_ascvd_risk > optimal_thresh,
         detection = blind_screened * race_aware_ascvd_risk) %>%
  group_by(race) %>%
  summarize(num_blind_screened = sum(blind_screened * wtmec8yr), 
            ...
            frac_blind_detections = num_blind_detections / sum(wtmec8yr * race_aware_ascvd_risk),
...
cvd_sensitivity_pareto_plot <- 
  ...
  ylab("Recall") +
  xlab("Fraction recommended statins") +
  scale_x_continuous(labels = scales::percent,
...
cvd_baseline_utility_overall <- baseline_utility_by_group %>%
  summarise(baseline_utility_overall = sum(average_baseline_utility * n) / sum(n))

cvd_baseline_utility_overall
...
race_aware_optimal_thresh <- optimize(compute_pop_utility, 
                                      ...
                                      risk_scores = data$race_aware_ascvd_risk,
                                      p_disease = data$race_aware_ascvd_risk,
...
race_blind_optimal_thresh <- optimize(compute_pop_utility, 
                                      ...
                                      screening_cost = screening_cost,
                                      utility_reward = utility_reward,
                                      risk_scores = data$race_blind_ascvd_risk,
                                      p_disease = data$race_aware_ascvd_risk,
                                      weights = data$wtmec8yr,
                                      maximum = TRUE)[1]$maximum
